...
RedisClient.prototype.connection_gone = function (why) {
    ...
    if (this.max_attempts && this.attempts >= this.max_attempts) {
        this.retry_timer = null;
         - some people need a "Redis is Broken mode" for future commands that errors immediately, and others
    ...
    this.retry_timer = setTimeout(function () {
        ...
        if (self.connect_timeout && self.retry_totaltime >= self.connect_timeout) {
            self.retry_timer = null;
             - engage Redis is Broken mode for future commands, or whatever
...
RedisClient.prototype.on_data = function (data) {
    ...
    try {
        ...
        // Callbacks that throw exceptions will land in return_reply(), below.
         - it might be nice to have a different "error" event for different types of errors
        this.emit("error", err);
    }
};
    ...
    if (command_obj && !command_obj.sub_command) {
        if (typeof command_obj.callback === "function") {
            if (this.options.detect_buffers && command_obj.buffer_args === false) {
             ...
             - confusing and error-prone that hgetall is special cased in two places
        ...
        if (Array.isArray(reply)) {
            ...
            if (type === "message") {
                ...
                // subscribe commands take an optional callback and also emit an event, but only the first response is included in the callback
                 - document this or fix it so it works in a more obvious way
                if (command_obj && typeof command_obj.callback === "function") {
...
Multi.prototype.exec = function (callback) {
    ...
    // drain queue, callback will catch "QUEUED" or error
     - get rid of all of these anonymous functions which are elegant but slow
    this.queue.forEach(function (args, index) {
     ...
     - make this callback part of Multi.prototype instead of creating it each time
